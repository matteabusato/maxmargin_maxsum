...
class MaxSum:
    ...
    def update_q_down(self):
        for mu in range(self.M):
            k_tilde = np.array([-1] * self.N)
            if self.I_minus[mu] is not None:
            ...
            for s_index in range(2):
                ...
                max1 = -np.inf
                for delta_under in self.POSSIBLE_DELTA_UNDERLINED:
                    index1 = self.delta_to_ind(delta_under + 1)
                    index2 = self.delta_to_ind(delta_under + s)
                    max1 = max(max1, self.psi_up[mu][index1] + self.psi_down[mu][index2])
                self.m_plus[mu][s_index] = max1
            ...
            for s_index in range(2):
                ...
                for delta_under in self.POSSIBLE_DELTA_UNDERLINED:
                    index1 = self.delta_to_ind(delta_under - 1)
                    index2 = self.delta_to_ind(delta_under + s)
                    val = self.psi_up[mu][index1] + self.psi_down[mu][index2]
                    if max1 < val:
                        max1 = val
                        argmax1 = delta_under
                self.delta_star[mu][s_index] = argmax1

                index1 = self.delta_to_ind(self.delta_star[mu][s_index] - 1)
                index2 = self.delta_to_ind(self.delta_star[mu][s_index] + s)
                self.m_minus[mu][s_index] = max1
                self.k_star[mu][s_index] = (self.delta_star[mu][s_index] - self.delta_tilde[mu] - 1) // 2
            ...
            for j in self.U_minus[mu]:
                if k_tilde[j] > max(self.k_star[mu]):
                    self.q_down[j][mu] = self.patterns[mu][j] * self.M_minus[mu]
                else:
                    m_hat = np.zeros(2)
                    for s_index in range(2):
                        s = self.ind_to_s(s_index)
                        index1 = self.delta_to_ind(self.delta_tilde[mu])
                        index2 = self.delta_to_ind(self.delta_tilde[mu] + 1 + s)
                        max1 = self.psi_up[mu][index1] + self.psi_down[mu][index2]

                        for k in range(1, int(self.k_star[mu][s_index]) + 1):
                            index1 = self.delta_to_ind(self.delta_tilde[mu] + 2 * k)
                            index2 = self.delta_to_ind(self.delta_tilde[mu] + 2 * k + 1 + s)
                            temp_sum = self.psi_up[mu][index1] + self.psi_down[mu][index2]
                            if k > k_tilde[j]:
                                temp_sum -= 2 * (
                                    np.abs(self.q_up[self.I_minus[mu][k]][mu]) - np.abs(self.q_up[j][mu])
                                )
                            max1 = max(max1, temp_sum)
                        m_hat[s_index] = max1 - np.abs(self.q_up[j][mu])
